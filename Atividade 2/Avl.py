...
class AVLTree(BST):
    ...
    def _autoCompletar(self, current_node, value, lista_retorno = []):
        if current_node is None:
            return 
        if str(current_node.value).startswith(value):
            lista_retorno.append(current_node.value)
        if value < current_node.value:
            self._autoCompletar(current_node.left_child, value, lista_retorno)
        self._autoCompletar(current_node.right_child, value, lista_retorno)
        return lista_retorno

    def autoCompletar(self, value):
        return self._autoCompletar(self.root, value)
